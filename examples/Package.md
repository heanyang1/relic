...
You can either move `mylib.lisp` to `lib` folder directory, or compile it to dynamically linked library and move it to `lib`:
```sh
...
The runtime will try the following things when the program calls `(import mylib)`:
1. Load `lib/mylib.relic` file and run `mylib` function, adding the symbols to the current environment.
2. If `lib/mylib.relic` is not found, JIT compile `lib/mylib.lisp` to add symbols.
...
- See [runtime.h](../c_runtime/runtime.h) for the complete C API.
- See [SDL2 package](sdl2/) for an example of creating (and using) C bindings.